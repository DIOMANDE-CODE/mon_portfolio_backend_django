...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for categorie in Categorie_Projet.objects.all():
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Catégorie '{categorie.nom_categorie}' mise à jour."
                )
            )
        ...
        for visuel in Visuel.objects.all():
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Visuel '{visuel.titre_visuel}' mis à jour.")
            )
        ...
        for projet in Projet.objects.all():
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Projet '{projet.titre_projet}' mis à jour.")
            )
